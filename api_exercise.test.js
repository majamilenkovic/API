...
const baseUrl =
  "https://l761dniu80.execute-api.us-east-2.amazonaws.com/default/";

describe("GET", () => {
  it("should return status code 200", async () => {
    const response = await request(baseUrl).get("/exercise_api");

    expect(response.statusCode).toBe(200);
  });
});

//It should be POST request, it creates resource
describe("PUT", () => {
  it("should set the new item", async () => {
    ...
    const response = await request(baseUrl).put("exercise_api").send({
      main_key: main_key,
      value: value,
    });
...
describe("DELETE", () => {
  it("should delete an item", async () => {
    const response = await request(baseUrl).delete("exercise_api/dolor");
...
describe("POST", () => {
  it("should update an item", async () => {
    ...
    const response = await request(baseUrl).post("/exercise_api").send({
